...
idl <- idl2021 %>% 
  filter(ageyear >= 110) %>%
  mutate(ltrunc = pmax(u110d, as.integer(c1 - bdate)) / 365.25,
...
rbind(with(north_europe, table(gender)),
...
u110y <- u110d / 365.25
NE_gender_gp <- with(north_europe,
     test_elife(time = nyears,
                thresh = u110y,
                ltrunc = ltrunc,
                rtrunc = rtrunc,
                family = "gp",
...
test_country_group_gp <- 
  with(bind_rows(europe %>% mutate(group = "europe"), 
...
north_europe_split <- 
  north_europe %>% 
  mutate(deathgr = ddate > ymd("2006-12-31"),
         c1 = if_else(ddate > ymd("2006-12-31"), ymd("2007-01-01"), c1),
         c2 = if_else(ddate <= ymd("2006-12-31"), ymd("2006-12-31"), c2),
