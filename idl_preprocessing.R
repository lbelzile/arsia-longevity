...
idl <- read.csv(file = "idl_complete.csv") %>%
  ...
  rename(ndays = AGEDAYS,
         ...
         ageyear = AGEYEARS,
         valid = VALIDATION) %>%
  mutate(country = factor(substr(IDNUMBER, 1, ifelse(substr(IDNUMBER, 1, 2) == "UK", 2, 3)))) %>% # Create country manually b/c one record is . (AUSTRIA)
  select(! IDNUMBER) %>%
  filter(! country  %in% c("JPN","USA")) %>% # USA doesn't record dates, only years
...
idl_USA <- read.csv(file = "idl_complete.csv") %>%
  ...
  mutate(country = factor(substr(IDNUMBER, 1, ifelse(substr(IDNUMBER, 1, 2) == "UK", 2, 3)))) %>% # Create country manually b/c one record is . (AUSTRIA)
  select(! IDNUMBER) %>%
  filter( country  %in% "USA") %>% # USA doesn't record dates, only years
...
idl <- idl %>% mutate(ltrunc1 = as.integer(ltrunc1),
...
idl  <- idl %>%
  mutate(ltrunc1 = if_else(country == "US", pmax(40117L, as.integer(c1 - bdate)), ltrunc1),
