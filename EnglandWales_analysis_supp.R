...
prof_exp_scale <- function(mle = NULL,
  ...
  conf_interv_fn <- function(object,
    ...
    fit.r <- stats::smooth.spline(x = na.omit(cbind(
      object$r,
      object$psi
    )), cv = FALSE)
    return(fit.r)
  }
  conf_interv_fn <- conf_interv_fn(list(
    psi = psi,
  ...
  function(B = 9999L,
    ...
    if (!missing(lower) && !missing(upper)) {
      if (length(upper) != 1 && length(upper) != 1) {
        stopifnot(
          "`upper` and `lower` should be vectors of the same length." = length(lower) == length(upper),
