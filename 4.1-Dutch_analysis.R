...
dutch1 <- dutch %>%
  subset(!is.na(ndays)) %>% # Remove interval censored data
...
g1 <- tstab_cp$g2 +
  ...
  geom_segment(data = nc_shape_df,
               aes(x = xlow,
                   xend = xhigh,
                   y = ylow,
                   yend = ylow), alpha = 0.4, lty = 2) +
  geom_segment(data = nc_shape_df,
               aes(x = xlow,
                   xend = xhigh,
                   y = yhigh,
                   yend = yhigh), alpha = 0.4, lty = 2) +
  scale_y_continuous(
...
logpost_fn <- function(par, thresh, family){
  with(dutch_data, lpost_elife(par, time = time,
                          time2 = time2,
                          event = event,
                          type = "interval",
                          thresh = thresh,
...
gomp_penult_df[[i]] <- tibble(
  ...
  y = c(sapply(1:nB, function(ind){
    gomp_penult_fn(x = seq(from = th[j], to = 109, length.out = 101),
...
g2 <- gomp_penult_summary_df  %>%
  ggplot() +
  geom_lineribbon(mapping = aes(x = x,
                                y = y,
                                ymin = .lower,
                                ymax = .upper,
                                col = threshold,
  ...
  geom_text(data = tibble(n = unlist(lapply(fit_gomp, nobs)),
                          x = th,
...
for(i in seq_along(th)){
# Cohort effect
  bcohort_dutch[[i]] <- with(dutch_data %>% filter(time > th[i]),
     test_elife(covariate = factor(cut(byear, cohorts_cut)),
                time = time,
                time2 = time2,
                event = event,
                type = "interval",
