...
exp_haz_df <- tibble(
  ...
  y = c(sapply(1:nB, function(ind) {
    hazard_fn_elife(x = xseq, par = postsamp_exp$sim_vals[ind, 1], family = "exp")
  }))
)
gomp_haz_df <- tibble(
  draw = rep(1:nB, each = length(xseq)),
  x = rep(xseq + th_haz, length.out = length(xseq) * nB),
  y = c(sapply(1:nB, function(ind) {
    hazard_fn_elife(x = xseq, par = postsamp_gomp$sim_vals[ind, ], family = "gomp")
  }))
)
gp_haz_df <- tibble(
  draw = rep(1:nB, each = length(xseq)),
  x = rep(xseq + th_haz, length.out = length(xseq) * nB),
  y = c(sapply(1:nB, function(ind) {
    hazard_fn_elife(x = xseq, par = postsamp_gp$sim_vals[ind, ], family = "gp")
  }))
)
