...
tdeath <- as.numeric(c(
  ymd("2015-04-01") - ymd("1898-03-05"),
  ymd("2013-06-12") - ymd("1897-04-19")
)) / 365.25
# Merge databases
japan_dat <- rbind(
  japan_dat,
...
rtrunc_r_sim <- rep.int(
  x = ifelse(jap_sim$gender == "male",
    2013, 2015
  ) - byear_sim,
  times = jap_sim$count
)
...
for (i in seq_len(B)) {
  ...
  rtrunc_sim <- with(
    ...
    ifelse(rep(gender == "male", times = count),
      2013, 2015
    ) - (bday / 365 + rep(byear, times = count))
  )
  samp_sim <- samp_elife(
    n = n_sim,
...
g2 <- ggplot(data = data.frame(
  endpoint = with(boot, c(
    ifelse(f_par_shape > 0, Inf, th_sim - f_par_scale / f_par_shape),
